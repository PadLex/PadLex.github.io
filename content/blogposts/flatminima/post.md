...
The Flat Minima Hypothesis states that models that converge to a flatter basin tend to generalize better to unseen data. It's a powerfull idea. Perhaps it's a usefull framework for understanding why the Muon optimizer works so well. Does it implicitly favores flatter minima? However, in this blog post, as we'll empirically verify the hypothesis myself, find a much more complicated (and more interesting) story than one might expected.
...
The original formulation for the Flat Minima Hypothesis is often (incorrectly?) credited to a 1997 paper by @hochreiter1997flat (conference version is from 1994 [@hochreiter1994simplifying]).  They proposed a training algorithm which converges to flatter minima and, crucially, argued that flat minima are "fat maxima" of the Bayesian posterior. However, the view that larger minima generalize better actually predates their work. %%startfold%% Bayesians already preferred posterior maxima with more probability mass [@buntine1991bayesian], and @hinton1993keeping had already argued in 1993 that a network whose parameters tolerate more noise is simpler as it can be written down with fewer bits, and simpler models should generalize better [@wallace1968information; @rissanen1978modeling]. Even though the Flat Minima Hypothesis does not have a clearcut formulation or a single first proponent, we can still broadly decompose it into three statements: (1) some models are simpler than others in the information-theoretic sense, (2) we can measure how simple they are from the geometry of the loss landscape around them, and (3) these simpler models should generalize better.

Initially the idea didn't catch on; the 1997 paper only drew a handful of citations a year for two decades. In large part, because Hochreiter and Schmidhuber's put forward a training algorithm rather than a measure. They based it on a non-local definition of wide minima as a "large connected region in weight-space where the error remains approximately constant". While intuitive, this definition is computationally intractable, and fundamentally ill-posed for modern models, where, due to overparametrization, all the minima you converge to tend to be connected by low-loss paths [@garipov2018loss; @draxler2018essentially].

The hypothesis only got a second life 20 years later, when @keskar2017large picked it up (crediting @hochreiter1997flat) while trying to explain why small training batches tend to generalize better than large ones. However, before they could make that connection, they had to propose a new definition of flatness. They coined a local measure of curvature called Sharpness. Typically, it's defined as the largest eigen value of the Hessien matrix of the model's loss. Sometimes in the litterature people use the mean of the eigen values rather then their maximum. Intuitively, either measure of curvature will be smaller if the model converges at the center of a wide flat minima and largeer in a sharp minima. Using this measure, Keskar et al. empirically found that larger batches converge to sharper minima. They also observed that sharpness is inversly correlated with generalization, and thus argued that the Flat Minima Hypothesis could explain why large batche sizes generalize poorly.
%%endfold%%
...
This seems easy enough to test. We can train a few hundred NNs on a simple, but realistic, task like CIFAR-10. Lucky for us, Keller Jordan's cifar10-airbench [@jordan2024airbench] offers just the scaffold we need to train them quickly. I got about 11s of wall clock per run on a Hopper GPU, and that includes (my poorly optimized) sharpness measurements after every epoch. Then, for each run, we can simply measure how well the model generalizes, and test if the gap between training and validation accuracy correlates with landing in a sharper loss region.
...
Most neural network architectures have symmetries along which you can re-parametrize the model without affecting its behaviour. Some symmetries, like rescaling between layers, also affect sharpness. If we halve the weights of one ReLU layer and double the weights of the next, the network computes exactly the same function, but the curvature along the halved layer's directions quadruples. @dinh2017sharp showed that any minimum can be re-parametrized to be arbitrarily sharp without changing how the model generalizes. In this light, the hypothesis, as formulated by @keskar2017large, is false. Perhaps the clustering we saw then simply shows that SGD, Adam and Muon each have a different biases for parameter scale.
...
This looks much better now! Runs from the different optimizers land on far more similar adaptive sharpness values (Muon's still sit about 2x lower than SGD and Adam before accounting for generalization) and adaptive sharpness correlates even more strongly with the generalization gap then raw sharpness ($r = 0.45$–$0.69$, versus $0.27$–$0.49$).

## Questioning the Hypothesis
Why does the hypothesis only hold with a fixed learning rate though?

Within each optimizer, the generalization gap has near-0 variance. It seems like using a learning rate scheduler leads to more consistent (though not better) generalization. So there's no variance to explain, but if that where the whole story, there should also be near-0 variance in the sharpness, which is not the case.

Or could we just be reading tea leaves? Maybe lower sharpness is just a proxy for a model that converged further, and convergence actually causes the gap? We can test this. If we control for training accuracy, the correlations under a fixed-learning-rate actually stronger (Coupled Adam's adaptive sharpness goes from $r = 0.67$ to $0.72$), while the scheduled runs stay uncorrelated. In fact, sharpness doesn't even correlate strongly with training accuracy (|r| < 0.3 for every optimizer, under either schedule).
...
## Promise
[TODO: While it's not complete, the hypothesis clearly holds a lot of potential! In fixed-learning-rate experiment, where able to say something about how well a model would generalize to validation data based on the gemoetry of the training loss landscape alone! That's very powerful if somone can work out the kinks. Maybe flatness isn't it in all cases, and there's some more general geometric variable tha could robustly track generalization in all cases? If you could find it, you might just have a breakthrough on your hands.]
