...
The Flat Minima Hypothesis states that models that converge to a flatter basin tend to generalize better to unseen data. When I first learned of it, I wondered if it could be a useful framework for understanding the Muon optimizer. Perhaps it performs so well by implicitly favoring flater minima? However, when I tried to empirically verify the hypothesis myself, I found a much more complicated (and more interesting) story then I had expected.
...
## Intuition
A common intuition for the Flat Minima Hypothesis is that the validation loss landscape can be approximated as a stochastically perturbed version of the training landscape. If a model converges to a wider/flatter basin in the training landscape, it feels intuitive that it would be more likely to stay within that same basin if the validation-loss landscape were slightly shifted.
...
## Promises
The original formulation for the Flat Minima Hypothesis is often credited to a 1997 paper by @hochreiter1997flat (conference version is from 1994 [@hochreiter1994simplifying]). They proposed a training algorithm which converges to flatter minima and, crucially, argued that flat minima are "fat maxima" of the Bayesian posterior. However, the view that larger minima generalize better actually predates their work. Bayesians already argued that posterior maxima with more probability mass [@buntine1991bayesian] generalize better, and @hinton1993keeping had argued in 1993 that a network whose weights tolerate noise can be written down with fewer bits, and simpler models should generalize better; Hochreiter and Schmidhuber's contribution was to point out that noise-tolerant weights and flat minima are the same thing. At the end of the day though, the core promise of the hypothesis is that (1) some models are simpler then others, (2) we can measure how simple they are based on the local geometry of the loss landscape, and (3) these simpler models should generalize better (which has been a central dogma of information-theoretic statistics for even longer [@wallace1968information; @rissanen1978modeling].) [TODO justify why it fell out of favour if it did, or if it didn't fall out of favour change the working of second life to just say it got it's spot in the lime light]

The hypothesis got a second life when @keskar2017large simplified it (crediting it to @hochreiter1997flat), and used it to explain why large training batches generalize worse than small ones; which was and still is an important question. They showed that large batches converge to sharper minima, and argued that the Flat Minima Hypothesis could explain why they didn't generalize as well. As a tractable proxy of flatness Hochreiter's boxes, they measured sharpness: (approximately) how much the loss can grow in a small neighborhood around the solution. That's a single number you can compute for any trained model, no bespoke training required. Suddenly the hypothesis was testable at scale, and for a while it looked great: comparing some 40 generalization measures head to head, @jiang2020fantastic found sharpness-based ones to be the most predictive of all.
...
This seems easy enough to test. We can train a few hundred CNNs on a simple, but realistic, task like CIFAR-10. Lucky for us, Keller Jordan's cifar10-airbench offers just the scaffold we need to train them quickly. I got about 11s of wall clock per run on a Hopper GPU, and that includes (my poorly optimized) sharpness measurements after every epoch. Then, for each run, we can simply measure how well the model generalizes, and test if it correlates with landing in a flatter (less sharp) loss region.
